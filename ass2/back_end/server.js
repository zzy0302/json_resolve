...
function decode(str) {
    if(str) {
        //console.log(Buffer.from(str, 'base64').toString().split('.')[1]);
        return Buffer.from(str, 'base64').toString().split('.')[1];
    }
    else
        return 'e';
}
...
server.post('/request/films/get', (req, res) => {
    console.log('[server] POST: /request/films/get');
    ...
    connection.query(`select movie_key, movie_name, movie_poster, movie_casts, movie_directors, movie_genres, movie_year from filmstable`, (err, t) => {
                if (err) {
                    console.log('Get all class sql error');
                    op.opstatus = 2;
                    opInsert(op);
                    connection.end();
                    return res.json({
                ...
                t.map((item) => {
                    ...
                    result.push({
                        ...
                        tname: item.tname,
                        cday: item.day.split(','),
                        cstarttime: item.starttime,
                        cendtime: item.endtime,
                        ctime: item.day + ' : ' + item.starttime + '~' + item.endtime,
                        campus: item.campus,
                        croom: item.croom,
                        cstartdate: item.cstartdate,
                        cenddate: item.cenddate,
                        cdate: item.cstartdate + '~' + item.cenddate,
                        cstatus: item.cstatus === 1 ? "正常" : "已关闭",
                        ccost: item.ccost,
                        ccapacity: item.ccapacity,
                        coperation: `<a href="#/admin/class/detail/${item.cid}/${item.cname}">查看详情</a> `
                    });
                });
            });
;
//教务端/新建课程信息 finished
server.post('/request/class/create', (req, res) => {
    console.log('[server] POST: /request/class/create');
    let args = req.body;
    let connection = mysql.createConnection(dbConnectionInfo);
    let op = ({
        id: decode(args.user),
...
//教务端/关闭课程 finished
server.post('/request/class/close', (req, res) => {
    console.log('[server] POST: /request/class/close');
    let args = req.body;
    let connection = mysql.createConnection(dbConnectionInfo);
    let idList = '';
